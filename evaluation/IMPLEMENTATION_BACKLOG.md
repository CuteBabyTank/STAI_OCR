...
- **Preserve existing working tests and features.** 332 tests pass at HEAD; every task
  below must leave that green.
...
### P0-1 · Refresh the W0 configuration freeze and capture a real run
...
### P0-2 · Results directory and a machine-readable writer
...
| **Command** | `./.venv/bin/python -m evaluation.report --help` |
| **Live model** | No |
| **Labeling** | No |
| **Difficulty** | S |
...
### P0-3 · Unit-test `extraction.reconcile` (unsupported/hallucinated total)
...
### P0-4 · Execute the trajectory harness against a live model
...
| **Command** | `./.venv/bin/python -m evaluation.trajectory --cases evaluation/datasets/trajectory_cases.json --out evaluation/results/raw/` |
| **Live model** | **Yes** |
| **Labeling** | No |
...
### P1-1 · Budget aggregation and carry-forward tests
...
### P1-2 · Drive the real loop guard, clarify path, and step limit
...
### P1-3 · PDF page expansion and batch failure isolation
...
|---|---|---|---|---|---|
| **P2-1** | Field-confidence and value-equality gating tests | new `evaluation/tests/test_w2a_confidence.py` | `core._logprob_token_spans` and the value-equality gate exercised with synthetic logprobs; currently zero coverage | No | M |
| **P2-2** | Quick Chat differential test (server vs client) | new shared corpus JSON read by both `parseQuick.test.ts` and `test_w2c_quick_python.py` | Both parsers produce identical drafts over one shared corpus; divergence fails a test in both suites | No | S |
| **P2-3** | `extraction._coerce_json` and `_num` tests | `evaluation/tests/` | Fence-wrapped, prose-wrapped, and truncated model output; numeric strings with ₱, commas, parentheses | No | S |
...
| **P2-5** | Direct `save_receipt` line-item linkage test | `evaluation/tests/` | Asserts line items are written and linked to the parent receipt; currently only exercised indirectly via the seeder | No | S |
| **P2-6** | Currency and category preservation on posting | `evaluation/tests/test_w2b_finance.py` | The one partial W2-E item closed | No | S |
...
| **P3-2** | Regression guard for doc/code drift | A test asserting `evaluation/README.md`'s test counts match collected counts would have caught the 39-vs-52 staleness |
